...
def prepare_training_data(config):
    ...
    elif 'dataset_type' in config and config['dataset_type'] == 'excel':
        train_set = ExcelDataset(config, mode='train')
    else:
...
def main():
    ...
    if config['lmdb']:
        config['dataset_json_folder'] = 'xxx'
    # create logger
    ...
    for epoch in range(config['start_epoch'], config['nEpochs'] + 1):
        if config.get('sampler', None) == 'slide':
            if epoch <= train_data_loader.sampler.T0:
