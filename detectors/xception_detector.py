...
class XceptionDetector(AbstractDetector):
    ...
    def build_backbone(self, config):
        ...
        #     r=4,                     # rank
        #     lora_alpha=8,            # 缩放
        #     target_modules=["pointwise", "depthwise"],  # 名称关键字匹配
        #     lora_dropout=0.05,
        #     bias="none",             # 卷积层一般不用 bias
        #     task_type="IMAGE_CLASSIFICATION",  # PEFT 0.10+ 直接写字符串
        # )
        # backbone = get_peft_model(backbone, lora_cfg)
        # backbone.print_trainable_parameters()   # 确认参数量已下降
        return backbone
