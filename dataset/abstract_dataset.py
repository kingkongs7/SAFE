...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    # def init_data_aug_method(self):
    #     # 可选：padding 大小，可根据最大 rotate_limit 估算
    #     pad = self.config['data_aug'].get('pad', 32)
    #     resolution = self.config['resolution']
    #     with_landmark = self.config['with_landmark']

    #     transforms = []

    #     if with_landmark:
    #         # 先 pad，让旋转时不会把关键点弄到原图外
    #         transforms.append(
    #             A.PadIfNeeded(
    #                 min_height=resolution + pad * 2,
    #                 min_width= resolution + pad * 2,
    ...
    #     # 如果没有 landmark，就做等比缩放；有 landmark 则跳过
    #     transforms.append(
    #         A.OneOf([
    #             IsotropicResize(
    #                 max_side=resolution,
    ...
    #     # 如果做了 pad，最后再 crop 回原始大小
    #     if with_landmark:
    #         transforms.append(
    #             A.Crop(
    #                 x_min=pad, y_min=pad,
    #                 x_max=pad + resolution,
    #                 y_max=pad + resolution,
    #                 always_apply=True
    #             )
    #         )
    ...
    #             format='xy',
    #             remove_invisible=False,  # 不删除可见性为 0（超界）的点
    #         ) if self.config['with_landmark'] else None
    #     )
