...
class Trainer(object):
    ...
    def train_step(self,data_dict):
        if self.config['optimizer']['type']=='sam':
            ...
            predictions = self.model(data_dict) # image, 
    ...
    def train_epoch(
        ...
        if epoch>=2:
            ...
            times_per_epoch = config['test_num']
        ...
        print(epoch, '==================>', len(train_data_loader))
        for iteration, data_dict in tqdm(enumerate(train_data_loader), total=len(train_data_loader)):
            ...
            for key in data_dict.keys():
                if data_dict[key]!=None and key!='name' and key!='text_label':
            ...
            if (step_cnt+1) % test_step == 0:
                if test_data_loaders is not None and (not self.config['ddp'] ):
                    ...
                    temp = None
                    if self.config['model_name'] == 'clip_aaai_f2':
                        temp = self.model.mode
                        self.model.mode = 'cls'
    ...
    def get_losses(self, train_dataset, config):
        self.model.eval()
        losses_dataloader = torch.utils.data.DataLoader(
        ...
        loss_buf = torch.empty(len(train_dataset), dtype=torch.float32)
        idx = 0
        with torch.inference_mode():
            for index, batch in tqdm(enumerate(losses_dataloader), total=len(losses_dataloader)):
                for k, v in batch.items():
                    if torch.is_tensor(v):
                        batch[k] = v.to(device, non_blocking=True)

                out = self.model(batch)["cls"]       
                loss = ce(out, batch["label"])      

                bsz = loss.size(0)
                loss_buf[idx: idx + bsz] = loss.cpu()  
                idx += bsz
        if self.config['model_name'] == 'clip_aaai_f2':
            self.model.mode = 'dual'
        return loss_buf.tolist()
